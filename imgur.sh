...
while [ $# -gt 0 ]; do
	...
	secure=$(echo $url | sed 's/^http:/https:/')
	sensible-browser "$secure" &
	sensible-browser "https://imgur.com/delete/$delete_hash" &
	backupdir=~/backup_pictures; [ -d "$backupdir" ] || mkdir $backupdir
	mv $file $backupdir
